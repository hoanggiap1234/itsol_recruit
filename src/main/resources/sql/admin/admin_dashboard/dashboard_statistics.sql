with time_filtered_job as (
   ...
   , all_job as (
    ...
    where js.code in ('RECRUITING', 'STOP', 'COLSED')
   ...
   , not_done_job_temp as (
    ...
    from time_filtered_job j
             ...
             left join status_job_register ps on ps.id = jr.status_id and ps.code = 'SUCCESS'
   ...
   , waiting_for_interview as (
    ...
    from time_filtered_job j
             ...
             inner join status_job_register ps on ps.id = jr.status_id and ps.code = 'INTERVIEW'
)
   , interviewing as (
    select count(jr.id) interviewing
    from time_filtered_job j
             inner join job_register jr on jr.job_id = j.id
             inner join status_job_register ps on ps.id = jr.status_id and ps.code = 'ACCEPTED_INTERVIEW'
)
   , total_apply as (
